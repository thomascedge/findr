...
async def test_create_group_chat_success(
    client: AsyncClient, auth_headers, test_user_2, db
):
    ...
    response = await client.post(
        "/api/v1/chats/",
        json=[str(test_user_2.id), str(user3.id)],
...
@pytest.mark.asyncio
async def test_create_group_chat_too_few_members(
    client: AsyncClient, auth_headers, test_user_2
):
    """Creating a group chat with fewer than 2 members returns 400."""
    response = await client.post(
        "/api/v1/chats/", json=[str(test_user_2.id)], headers=auth_headers
    )
    assert response.status_code == 400


@pytest.mark.asyncio
async def test_create_group_chat_nonexistent_user(
    client: AsyncClient, auth_headers, test_user_2
):
    """Creating a group chat with a non-existent user ID returns 404."""
    fake_id = "00000000-0000-0000-0000-000000000000"
    response = await client.post(
        "/api/v1/chats/", json=[str(test_user_2.id), fake_id], headers=auth_headers
    )
...
async def test_get_chat_members_success(
    ...
    response = await client.get(
        f"/api/v1/chats/{chat_id}/members", headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_get_chat_members_non_member_rejected(
    client: AsyncClient, auth_headers, auth_headers_2, test_user_2, db
):
...
@pytest.mark.asyncio
async def test_get_chat_members_excludes_inactive(
    client: AsyncClient, auth_headers, test_user_2, db
):
...
async def test_add_member_success(
    ...
    response = await client.post(
        f"/api/v1/chats/{chat_id}/members?user_id={user4.id}", headers=auth_headers
    )
    assert response.status_code == 200


@pytest.mark.asyncio
async def test_add_member_already_in_chat(
    client: AsyncClient, auth_headers, test_user_2, db
):
...
async def test_non_member_cannot_add(
    ...
    response = await client.post(
        "/api/v1/auth/token", data={"username": "user4", "password": "password123"}
    )
...
async def test_remove_member_success(
    ...
    response = await client.delete(
        f"/api/v1/chats/{chat_id}/members/{test_user_2.id}", headers=auth_headers
    )
    assert response.status_code == 200


@pytest.mark.asyncio
async def test_remove_admin_rejected(
    client: AsyncClient, auth_headers, test_user_2, db
):
    ...
    result = await db.execute(
        select(ChatMember).where(
            ChatMember.chat_id == chat_id, ChatMember.user_id == test_user_2.id
        )
    )
...
@pytest.mark.asyncio
async def test_non_member_cannot_remove(
    client: AsyncClient, auth_headers, test_user_2, db
):
...
@pytest.mark.asyncio
async def test_transfer_admin_success(
    client: AsyncClient, auth_headers, test_user_2, db
):
    ...
    response = await client.patch(
        f"/api/v1/chats/{chat_id}/members/admin/{test_user_2.id}", headers=auth_headers
    )
    assert response.status_code == 200


@pytest.mark.asyncio
async def test_transfer_admin_non_admin_rejected(
    client: AsyncClient, auth_headers, auth_headers_2, test_user_2, db
):
    ...
    response = await client.patch(
        f"/api/v1/chats/{chat_id}/members/admin/{user3.id}", headers=auth_headers_2
    )
    assert response.status_code == 403


@pytest.mark.asyncio
async def test_transfer_admin_to_non_member(
    client: AsyncClient, auth_headers, test_user_2, db
):
    ...
    response = await client.patch(
        f"/api/v1/chats/{chat_id}/members/admin/{user4.id}", headers=auth_headers
    )
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_transfer_admin_to_already_admin(
    client: AsyncClient, auth_headers, test_user_2, db
):
