...
@pytest.mark.asyncio
async def test_update_location_twice_upserts(
    client: AsyncClient, auth_headers, db, test_user
):
    ...
    await client.put(
        "/api/v1/location/me",
        json={"lat": 30.2, "lng": -97.7, "visible": True},
        headers=auth_headers,
    )
    await client.put(
        "/api/v1/location/me",
        json={"lat": 30.3, "lng": -97.8, "visible": True},
        headers=auth_headers,
    )

    result = await db.execute(
        select(UserLocation).where(UserLocation.user_id == test_user.id)
    )
...
@pytest.mark.asyncio
async def test_get_nearby_invisible_user_excluded(
    client: AsyncClient, auth_headers, auth_headers_2
):
    """An invisible user does not appear in nearby results."""
    # Set user 2 location but invisible
    await client.put(
        "/api/v1/location/me",
        json={"lat": 30.2672, "lng": -97.7431, "visible": False},
        headers=auth_headers_2,
    )
    # Set user 1 location
    await client.put(
        "/api/v1/location/me",
        json={"lat": 30.2672, "lng": -97.7431, "visible": True},
        headers=auth_headers,
    )
