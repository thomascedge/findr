...
@pytest.mark.asyncio
async def test_send_dm_reuses_existing_chat(
...
async def test_send_message_no_recipient_or_chat(client: AsyncClient, auth_headers):
    ...
    response = await client.post(
        "/api/v1/messages/send", json={"body": "Hello!"}, headers=auth_headers
    )
    assert response.status_code == 400


@pytest.mark.asyncio
async def test_send_message_both_recipient_and_chat(
    client: AsyncClient, auth_headers, test_user_2
):
...
@pytest.mark.asyncio
async def test_send_message_to_inactive_user(
...
@pytest.mark.asyncio
async def test_send_message_body_too_long(
    client: AsyncClient, auth_headers, test_user_2
):
...
async def test_get_thread_success(client: AsyncClient, auth_headers, test_user_2):
    ...
    response = await client.get(
        f"/api/v1/messages/thread?recipient_id={test_user_2.id}", headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_get_thread_excludes_deleted(
    client: AsyncClient, auth_headers, test_user_2
):
...
async def test_edit_someone_elses_message(
    client: AsyncClient, auth_headers, auth_headers_2, test_user_2, test_user
):
...
@pytest.mark.asyncio
async def test_delete_someone_elses_message(
    client: AsyncClient, auth_headers, auth_headers_2, test_user_2
):
    ...
    response = await client.delete(
        f"/api/v1/messages/{message_id}", headers=auth_headers_2
    )
    assert response.status_code == 403
