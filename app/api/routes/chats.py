...
async def create_group_chat(
    ...
    if len(member_ids) < 2:
        raise HTTPException(
            status_code=400,
            detail="Group chat cannot be created with less than 2 users.",
        )
...
async def get_chat_members(
    ...
    result = await db.execute(
        ...
        .where(ChatMember.chat_id == chat_id, User.is_active == True)
    )
...
async def add_member(
    ...
    if user_member:
        raise HTTPException(
            status_code=400, detail="User is already a member of this chat."
        )
...
async def remove_member(
    ...
    if not chat_member:
        raise HTTPException(
            status_code=404, detail="User is not a member of this chat."
        )
...
async def transfer_admin_access(
    ...
    if not current_member or not current_member.is_admin:
        raise HTTPException(
            status_code=403, detail="You must be an admin to perform this action."
        )
